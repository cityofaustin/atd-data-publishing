...
import arrow
import requests
from socrata_helpers import upsert_data, prep_pub_log
import secrets
...
if dropbox_month == socrata_month:
    ...
    log_entry = prep_pub_log(arrow.now(), 'bcycle_trip_update', upsert_res)
    upsert_data(socrata_creds, log_entry, resource_id_pub_log)
    ...
    if data_exists_dropbox(path, access_token):
        ...
        upsert_res = upsert_data(socrata_creds, data, resource_id_publish)
